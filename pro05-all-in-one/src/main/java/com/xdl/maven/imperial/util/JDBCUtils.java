package com.xdl.maven.imperial.util;
...
/**
 * 功能1：从数据源获取数据库连接
 * 功能2：将数据连接绑定到本地线程（借助ThreadLocal）
 * 功能3：释放线程时和本地线程解除绑定
 */
public class JDBCUtils {
    ...
    static {
        try {
            ...
            // 为了保证程序代码的可移植性，需要基于一个确定的基准来读取文件
            // 确定的基准：类路径的根目录。resources目录下的内容经过构建操作中的打包操作后会确定放在WEB-INF/classes目录下。
            // WEB-INF/class 目录存放编译好的*.class字节码文件，所以这个目录我们称之为类路径。
            ...
            //4、根据properties对象（已经封装了数据源连接信息）来创建数据源对象
